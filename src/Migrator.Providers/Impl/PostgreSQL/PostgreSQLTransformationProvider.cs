    ...
    public class PostgreSQLTransformationProvider : TransformationProvider
    {
        ...
        public override bool ConstraintExists(string table, string name)
        {
            using (IDataReader reader =
                ExecuteQuery(string.Format("SELECT constraint_name FROM information_schema.table_constraints WHERE constraint_name = lower('{0}')", name)))
            {
                return reader.Read();
            }
        }
        ...
        public override bool ColumnExists(string table, string column)
        {
            ...
            using (IDataReader reader =
                ExecuteQuery(String.Format("SELECT column_name FROM information_schema.columns WHERE table_name = lower('{0}') AND column_name = lower('{1}')", table, column)))
            {
                return reader.Read();
            }
        }
        ...
        public override bool TableExists(string table)
        {
            ...
            using (IDataReader reader =
                ExecuteQuery(String.Format("SELECT table_name FROM information_schema.tables WHERE table_name = lower('{0}')", table)))
            {
                return reader.Read();
            }
        }
